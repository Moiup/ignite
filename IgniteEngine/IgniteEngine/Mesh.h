...
private:
	...
	std::vector<uint32_t> _indices_to_mat; // Given a a vertex index (from indices), provides the index of the corresponding material
...
public:
	...
	const void setIndicesToMaterial(uint32_t* indices_mat, const uint32_t len);
	...
	const std::vector<uint32_t>& getIndicesToMaterial() const;
	...
	uint32_t getIndicesToMaterialStride();
	...
	uint32_t getIndicesToMaterialNbElem();
